...
@router.get("/subjects/{subject_name}/sections", response_model=list[schemas.SectionResponse])
def get_sections_for_subject(subject_name: str, db: Session = Depends(get_db)):
    """
    GET /api/subjects/{subject_name}/sections
    Returns all sections and their files for a given subject name.
    Used by FilesLink.jsx to display the file cards grouped by section.

    Example: GET /api/subjects/DBMS/sections
    Returns: [ { title: "Unit wise PPT", files: [...] }, { title: "CIE", files: [...] }, ... ]
    """
...
def add_file_to_section(section_id: int, file: schemas.FileCreate, db: Session = Depends(get_db)):
    ...
    Adds a new file link to a specific section.
